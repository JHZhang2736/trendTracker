...
async def _score_batch(
    ...
    prompt = (
        f"用户画像：{user_profile}\n\n"
        "对以下热搜关键词逐条判断：该趋势对这个用户有没有价值？\n"
        "价值包括但不限于：\n"
        "- 直接相关的行业动态（科技、金融、电商等）\n"
        "- 能启发商业机会的趋势（如某种娱乐方式火了→可以卖相关产品/做内容）\n"
        "- 拓宽认知、值得了解的重大事件\n\n"
        "只过滤纯粹无价值的信息（如明星私生活、纯娱乐八卦、体育比分）。\n\n"
        f"{numbered}\n\n"
        "逐条判断，有价值的给分数0-100和一句话商业启发（20字内）。\n"
        '格式：[{"i":序号,"s":分数,"r":"商业启发"},...] 只含有价值条目。全无价值返回[]'
    )
