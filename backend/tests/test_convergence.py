...
def test_score_is_between_0_and_100():
    score = compute_convergence_score(
        heat_score=5000.0, rank=0, age_hours=0.0, platform_max_heat=10000.0
    )
...
def test_rank_1_scores_higher_than_rank_50():
    ...
    score_bottom = compute_convergence_score(
        heat_score=5000.0, rank=49, age_hours=0.0, platform_max_heat=10000.0
    )
...
def test_higher_heat_scores_higher():
    score_high = compute_convergence_score(
        heat_score=9000.0, rank=5, age_hours=0.0, platform_max_heat=10000.0
    )
    score_low = compute_convergence_score(
        heat_score=1000.0, rank=5, age_hours=0.0, platform_max_heat=10000.0
    )
    assert score_high > score_low


def test_platform_max_heat_normalizes_correctly():
    # Same absolute heat, different platform max → different scores
    score_high_max = compute_convergence_score(
        heat_score=5000.0, rank=5, age_hours=0.0, platform_max_heat=10000.0
    )
    score_low_max = compute_convergence_score(
        heat_score=5000.0, rank=5, age_hours=0.0, platform_max_heat=5000.0
    )
    # Lower max_heat means this keyword is at 100% of platform — scores higher
...
def test_older_trend_decays():
    ...
    score_old = compute_convergence_score(
        heat_score=5000.0, rank=5, age_hours=24.0, platform_max_heat=10000.0
    )
...
def test_12h_old_trend_roughly_half_score():
    score_fresh = compute_convergence_score(
        heat_score=10000.0, rank=0, age_hours=0.0, platform_max_heat=10000.0
    )
    score_12h = compute_convergence_score(
        heat_score=10000.0, rank=0, age_hours=12.0, platform_max_heat=10000.0
    )
...
def test_none_heat_score_does_not_crash():
    score = compute_convergence_score(
        heat_score=None, rank=1, age_hours=0.0, platform_max_heat=10000.0
    )
    assert 0.0 <= score <= 100.0
...
def test_none_rank_does_not_crash():
    score = compute_convergence_score(
        heat_score=5000.0, rank=None, age_hours=0.0, platform_max_heat=10000.0
    )
    assert 0.0 <= score <= 100.0
...
def test_zero_max_heat_returns_zero_or_low():
    score = compute_convergence_score(
        heat_score=5000.0, rank=None, age_hours=0.0, platform_max_heat=0.0
    )
    assert 0.0 <= score <= 100.0
...
@pytest.mark.asyncio
async def test_top_by_platform_returns_200(test_client: AsyncClient):
    resp = await test_client.get("/api/v1/trends/top-by-platform")
    assert resp.status_code == 200


@pytest.mark.asyncio
async def test_top_by_platform_schema(test_client: AsyncClient, db_session: AsyncSession):
    await _seed(db_session, "热词A", "weibo", 0, 9000.0)
    await _seed(db_session, "热词B", "google", 1, 5000.0)
    data = (await test_client.get("/api/v1/trends/top-by-platform")).json()
    assert "platforms" in data
    assert isinstance(data["platforms"], dict)


@pytest.mark.asyncio
async def test_top_by_platform_per_platform_items(
    test_client: AsyncClient, db_session: AsyncSession
):
    await _seed(db_session, "热词A", "weibo", 0, 9000.0)
    await _seed(db_session, "热词B", "weibo", 1, 5000.0)
    await _seed(db_session, "TrendC", "google", 0, 1000000.0)
    data = (await test_client.get("/api/v1/trends/top-by-platform")).json()
    platforms = data["platforms"]
    assert "weibo" in platforms
    assert "google" in platforms
    # Per-platform items should have required fields
    for item in platforms["weibo"]:
...
@pytest.mark.asyncio
async def test_top_by_platform_respects_limit(test_client: AsyncClient, db_session: AsyncSession):
    for i in range(15):
        await _seed(db_session, f"词{i}", "weibo", i, float((15 - i) * 1000))
    data = (await test_client.get("/api/v1/trends/top-by-platform?limit=5")).json()
    assert len(data["platforms"]["weibo"]) <= 5
